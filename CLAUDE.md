...
Quick Prosets — browser-based single-player ProSet card game. A card is six fixed-position coloured circles, each solid or open; three cards form a proset when every colour is solid an even number of times. Svelte 5 + Vite + TypeScript, deployed to GitHub Pages.
...
[src/App.svelte](src/App.svelte) composes `Header`, `CardGrid`, `GameOverModal`, `MenuModal`. (`Toast` is rendered inside `Header`.) App.svelte owns the cross-component lifecycle: instantiating `Game`, the `pendingAction` transition machine, the visibility listener, and writing animation CSS variables.
...
- `#checkBoard()` runs after dealing: tops up to `MIN_BOARD`, reshuffles via `#refresh()` when no proset exists on board, ends the game when board ∪ deck has no proset. End delivers `{ time, disqualified }` through the `onEndGame` dep, which transitions phase to `over`.
...
Other modules: [src/lib/game-utils.ts](src/lib/game-utils.ts) — pure logic. A `Card` is a bitmask over `COLORS` (bit i = that colour is solid); the deck is the 63 non-zero masks, since the all-open card can never complete a proset. `isValidProset` is `(a ^ b ^ c) === 0`, and `findProset` exploits the third card being forced to `a ^ b`. [src/lib/constants.ts](src/lib/constants.ts) — timings, board sizes, `MODE_TIMINGS`, card and circle-grid geometry. [src/lib/storage.ts](src/lib/storage.ts) — localStorage for top-5 scores, theme, mode.

Card visuals: [src/components/Card.svelte](src/components/Card.svelte) draws six `<circle>`s in a 2×3 grid laid out from the `DOT_*` constants, one slot per colour in `COLORS` order. Fill and stroke come from the `.dot` / `.color-*` rules at the bottom of [src/app.css](src/app.css), which resolve a `--dot-color` token themed separately for light and dark.
